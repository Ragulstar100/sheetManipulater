 ...
 tooltip: {
   ...
   let barChartBulider= (()=>{
          ...
          obj().playerName.forEach((e)=>{
            let local={"Games":e}
              data.forEach((g)=>{
              local[g[GameData.gameName]] = data.filter((f)=>f[GameData.gameName]==g[GameData.gameName]).reduce((acc,r)=>acc+parseInt(r[GameData.timePlayed].split(' ')[0]),0)
              })
            list.push(local)  
          })
...
function App() {
  ...
  return (
    ...
    <Dropdown className='self-end' data={gameFilterData(data).playerName} onSelect={(v)=>{
      setName(v)
    }}/>  
   ...
   <Chart className='w-screen h-150' data={dataFilter(data.filter((e)=>{
      return name=='All'||e[GameData.playerName]==name
    })).totalPlayedData}/>
